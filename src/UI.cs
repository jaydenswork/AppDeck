...
namespace AppDeck {
    ...
    internal sealed class CaptionBar:Control {
        readonly ModernForm owner;readonly List<IconButton> actions=new List<IconButton>();readonly IconButton minimize,maximize,close;public bool Compact;public string Subtitle="";public IconButton Leading;bool showMinimize=true,showMaximize=true,showClose=true;
        public CaptionBar(ModernForm form){owner=form;DoubleBuffered=true;minimize=new IconButton("min","最小化");maximize=new IconButton("max","最大化 / 还原");close=new IconButton("close","关闭窗口"){Danger=true};minimize.Click+=(s,e)=>owner.WindowState=FormWindowState.Minimized;maximize.Click+=(s,e)=>owner.ToggleMaximize();close.Click+=(s,e)=>owner.Close();Controls.Add(minimize);Controls.Add(maximize);Controls.Add(close);Theme.Watch(this,()=>{BackColor=Theme.Surface;Invalidate();});MouseDown+=(s,e)=>{if(e.Button==MouseButtons.Left&&e.Clicks==1)owner.BeginMove();};DoubleClick+=(s,e)=>{if(showMaximize)owner.ToggleMaximize();};owner.Resize+=(s,e)=>{maximize.Glyph=owner.WindowState==FormWindowState.Maximized?"restore":"max";maximize.Invalidate();};}
        public void SetWindowButtons(bool minimizeVisible,bool maximizeVisible,bool closeVisible=true){showMinimize=minimizeVisible;showMaximize=maximizeVisible;showClose=closeVisible;PerformLayout();Invalidate();}
        public int ConfiguredWindowButtonCount{get{return (showMinimize?1:0)+(showMaximize?1:0)+(showClose?1:0);}}
        public bool WindowButtonBoundsReady{get{return (!showMinimize||minimize.Width>=24)&&(!showMaximize||maximize.Width>=24)&&(!showClose||close.Width>=24);}}
        ...
        protected override void OnLayout(LayoutEventArgs le){base.OnLayout(le);float s=Height/44f;int w=(int)(Compact?34*s:40*s),right=Width;close.Visible=showClose;maximize.Visible=showMaximize;minimize.Visible=showMinimize;if(showClose){right-=w;close.SetBounds(right,0,w,Height);}if(showMaximize){right-=w;maximize.SetBounds(right,0,w,Height);}if(showMinimize){right-=w;minimize.SetBounds(right,0,w,Height);}right-=(int)(8*s);for(int i=actions.Count-1;i>=0;i--){right-=w;actions[i].SetBounds(right,0,w,Height);}if(Leading!=null)Leading.SetBounds(0,0,w,Height);}
        protected override void OnPaint(PaintEventArgs e){base.OnPaint(e);var g=e.Graphics;float s=Height/44f;g.SmoothingMode=SmoothingMode.AntiAlias;int left=(int)(14*s);if(Leading==null){PaintKit.Fill(g,new RectangleF(10*s,8*s,28*s,28*s),Theme.AccentFill,8*s);PaintKit.Icon(g,"logo",new RectangleF(15*s,12*s,19*s,19*s),Color.White);left=(int)(48*s);}else left=Leading.Right+5;
            int end=actions.Count>0?actions[0].Left-8:(showMinimize?minimize.Left:showMaximize?maximize.Left:showClose?close.Left:Width)-10;string text=owner.Text.Replace(" · AppDeck","");using(var f=Theme.Font(Compact?9:10,true))PaintKit.Text(g,text,f,Theme.Ink,new Rectangle(left,0,Math.Max(0,end-left),Height));using(var p=new Pen(Theme.Line))g.DrawLine(p,0,Height-1,Width,Height-1);}
    }
}
