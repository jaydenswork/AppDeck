...
namespace AppDeck {
    ...
    internal sealed class CaptionBar:Control {
        ...
        protected override void OnLayout(LayoutEventArgs le){base.OnLayout(le);float s=Height/44f;int w=(int)(Compact?34*s:40*s),right=Width;foreach(var b in new[]{close,maximize,minimize}){right-=w;b.SetBounds(right,0,w,Height);}right-=(int)(8*s);for(int i=actions.Count-1;i>=0;i--){right-=w;actions[i].SetBounds(right,0,w,Height);}if(Leading!=null)Leading.SetBounds(0,0,w,Height);}
        protected override void OnPaint(PaintEventArgs e){base.OnPaint(e);var g=e.Graphics;float s=Height/44f;g.SmoothingMode=SmoothingMode.AntiAlias;int left=(int)(14*s);if(Leading==null){PaintKit.Fill(g,new RectangleF(10*s,8*s,28*s,28*s),Theme.AccentFill,8*s);PaintKit.Icon(g,"logo",new RectangleF(15*s,12*s,19*s,19*s),Color.White);left=(int)(48*s);}else left=Leading.Right+5;
            int end=actions.Count>0?actions[0].Left-8:minimize.Left-10;string text=owner.Text.Replace(" · AppDeck","");using(var f=Theme.Font(Compact?9:10,true))PaintKit.Text(g,text,f,Theme.Ink,new Rectangle(left,0,Math.Max(0,end-left),Height));using(var p=new Pen(Theme.Line))g.DrawLine(p,0,Height-1,Width,Height-1);}
    }
}
